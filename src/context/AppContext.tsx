...
export const SECURE_ADMIN_PHONE = '97846 64518';
export const SECURE_ADMIN_NUMBER = '97846 64518';
export const SECURE_ADMIN_EMAIL = '9784664518@parthacademy.edu';
export const SECURE_ADMIN_BACKUP_EMAIL = 'garvit986sharma@gmail.com';
export const ADMIN_MASTER_PASSWORD = 'Parth2026';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Helper to get allowed admin passwords
  const isAuthorizedPassword = (inputPass: string): boolean => {
    const trimmed = inputPass.trim();
    if (!trimmed) return false;
    if (trimmed === ADMIN_MASTER_PASSWORD) return true;
    const custom = localStorage.getItem('pa_admin_custom_pwd');
    if (custom && custom.trim() === trimmed) return true;
    return false;
  };
  ...
  const loginAdmin = async (
    ...
    const rawId = (identifier || SECURE_ADMIN_PHONE).trim().toLowerCase();
    const cleanNum = rawId.replace(/\s+/g, '').replace(/^\+91/, '');
    const isAuthorized =
      cleanNum === '9784664518' ||
      rawId === SECURE_ADMIN_PHONE.toLowerCase() ||
      rawId === SECURE_ADMIN_EMAIL.toLowerCase() ||
      rawId === SECURE_ADMIN_BACKUP_EMAIL.toLowerCase() ||
      rawId.includes('garvit');

    if (!isAuthorized) {
      return {
        success: false,
        error: `Access Denied: Only the authorized director mobile number (${SECURE_ADMIN_PHONE}) has administrative rights.`,
      };
    }

    const trimmedInput = password.trim();

    // Check if matching master password 'Parth2026' or updated director password
    if (isAuthorizedPassword(trimmedInput)) {
      // Connect / ensure active Firebase Auth session so rules & sync work
      if (!auth.currentUser) {
        try {
          await signInWithEmailAndPassword(auth, SECURE_ADMIN_EMAIL, trimmedInput);
        } catch {
          try {
            await createUserWithEmailAndPassword(auth, SECURE_ADMIN_EMAIL, trimmedInput);
          } catch {
            try {
      ...
      setIsAdminAuthenticated(true);
      setAdminUser({
        id: auth.currentUser?.uid || 'admin-garvit-director',
        email: SECURE_ADMIN_PHONE,
        phone: SECURE_ADMIN_PHONE,
        name: 'Director Garvit Sharma',
        role: 'superadmin',
    ...
    // Attempt authentication across recognized accounts in Firebase Auth
    const testEmails = [SECURE_ADMIN_EMAIL, SECURE_ADMIN_BACKUP_EMAIL];
    let userCred: any = null;

    for (const testEmail of testEmails) {
      try {
        userCred = await signInWithEmailAndPassword(auth, testEmail, trimmedInput);
        if (userCred?.user) break;
      } catch (err: any) {
        // Continue to check next email candidate
      }
    }
    ...
    return {
      success: false,
      error: `Incorrect secret password for administrator ${SECURE_ADMIN_PHONE}. Please check your credentials.`,
    };
  };
  ...
  const setupAdminAccount = async (
    ...
    localStorage.setItem('pa_admin_custom_pwd', password.trim());

    try {
      const userCred = await createUserWithEmailAndPassword(auth, SECURE_ADMIN_EMAIL, password.trim());
      setIsAdminAuthenticated(true);
      setAdminUser({
        id: userCred.user.uid,
        email: SECURE_ADMIN_PHONE,
      ...
      // If already registered in Firebase, update local record and establish active session
      if (!auth.currentUser) {
        try {
          await signInWithEmailAndPassword(auth, SECURE_ADMIN_EMAIL, password.trim());
        } catch {
          try {
            await signInAnonymously(auth);
          } catch {
  ...
  const sendAdminPasswordReset = async (): Promise<{ success: boolean; error?: string }> => {
    try {
      await sendPasswordResetEmail(auth, SECURE_ADMIN_BACKUP_EMAIL);
      return { success: true };
    } catch (err: any) {
      try {
        ...
        let msg = 'Failed to send password reset notification.';
  ...
  const changeAdminPassword = async (
    ...
    localStorage.setItem('pa_admin_custom_pwd', trimmed);
    if (auth.currentUser) {
      try {
        await updatePassword(auth.currentUser, trimmed);
      } catch {
        // If re-auth needed, local token is securely persisted
      }
    }
    return { success: true };
  };
